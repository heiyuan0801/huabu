...
在 Vercel 中导入仓库即可。当前版本的 AI 请求默认由浏览器直连用户自己的 OpenAI 兼容地址；`weilai.chat` 渠道会自动使用项目内置的 Next.js 固定代理，不需要额外配置 CORS。
...
- AI API Key 保存在浏览器本地；默认由前端直连 OpenAI 兼容接口，`weilai.chat` 渠道请求会经过当前 Next.js 服务。
