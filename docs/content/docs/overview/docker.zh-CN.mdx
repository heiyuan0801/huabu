...
默认 `docker-compose.yml` 会从当前源码构建 `infinite-canvas:local`。
...
当前主应用镜像使用 nginx 托管 Vite 静态前端，并通过 `/ai-proxy` 同源代理默认未来中转站 `https://api1.weilai.chat`；HTTPS 部署下的远程生成图片通过 `/media-proxy` 访问。自定义 AI 渠道和第三方提示词仍由浏览器直连。画布、我的素材、生成记录和 AI API Key 默认保存在浏览器本地，不需要额外挂载数据目录。
