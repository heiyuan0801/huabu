...
- In Vite development and Docker deployments, requests for the default Weilai relay `https://api1.weilai.chat` use same-origin `/ai-proxy`; other channels remain browser-direct.
...
- API keys remain in browser storage and requests are sent directly to configured providers. The default Weilai relay uses same-origin proxying only in supported Vite and Docker deployments.
- HTTPS deployments rewrite generated media from the Weilai HTTP origin to same-origin `/media-proxy` so browsers do not block it as mixed content.
