...
- The AI API key is stored in the browser. Custom channels send requests directly to their configured providers, while Vite development and Docker deployments same-origin proxy the default Weilai relay.
- The default Weilai relay uses the same-origin `/ai-proxy` and `/media-proxy` paths in supported deployments.
- [How the Local Codex Connection Works](/docs/development/local-codex-canvas)
