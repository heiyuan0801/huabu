export default {
    ...
    ecommerce: {
        ...
        fallbackPrompt: "Create a vertical e-commerce detail-page section for this product. Product information: {{product}}. Section focus: {{focus}}. Image ratio: {{ratio}}. The referenced product must be the only product subject. Preserve its appearance, logo, color, material, and proportions exactly, and do not invent specifications. Use professional commercial photography, a clear layout, concise selling copy, and natural top and bottom transitions for vertical stitching.",
        planInstruction: "You are a senior e-commerce visual director. Use the supplied product image and product information to plan a continuous product detail page. Return only a JSON array with exactly {{count}} items. Each item must be {\"title\":\"short section name\",\"prompt\":\"complete prompt ready for the image model\"}. Each image uses a {{ratio}} ratio. Cover, in order, a premium hero, core benefits, structural or material details, authentic use cases, specifications/comparison or trust information, and a closing purchase-oriented scene; merge or expand these stages to fit the requested count. Every prompt must be self-contained, identify the referenced product as the only product subject, preserve its shape, logo, color, material, and proportions exactly, and never invent specifications. Define composition, setting, lighting, color, camera, layout, and concise on-image selling copy. Keep one visual system across the sequence and leave natural transitions for vertical stitching. Do not return Markdown, explanations, or text outside the JSON array.",
    },
    videoWorkbench: {
        title: "Video Studio",
