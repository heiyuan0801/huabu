export default {
    ...
    ecommerce: {
        ...
        fallbackPrompt: "为以下商品生成电商详情页竖图分镜。商品信息：{{product}}。本张主题：{{focus}}。画面比例 {{ratio}}。参考图中的商品是唯一产品主体，严格保持外观、Logo、颜色、材质和比例一致，不虚构参数；采用专业电商摄影与清晰版式，呈现简短中文卖点，并为上下长图拼接保留自然衔接。",
        planInstruction: "你是资深电商视觉策划。结合用户提供的商品主图与商品信息，规划一套可连续拼接的商品详情页。必须只输出一个 JSON 数组，恰好包含 {{count}} 项，每项格式为 {\"title\":\"简短中文分镜名\",\"prompt\":\"可直接交给图片模型的完整中文提示词\"}。单图比例为 {{ratio}}。分镜应依次覆盖：品牌感首屏、核心卖点、结构或材质细节、真实使用场景、参数/对比或信任信息、收尾购买氛围；根据实际张数合理合并或扩展。每条 prompt 都必须自包含，明确参考图中的商品是唯一产品主体，严格保持商品外观、Logo、颜色、材质和比例一致，不得虚构商品参数；写清构图、场景、光线、色彩、镜头、版式和可呈现的简短中文卖点，保持整套视觉语言统一，并为上下拼接预留自然衔接。不要输出 Markdown、解释或 JSON 之外的文字。",
    },
    videoWorkbench: {
        title: "视频创作台",
