...
+ [新增] 新增图片聊天页面，支持按对话生成图片、保存聊天记录及管理生成结果。
+ [新增] 图片聊天支持生成结果自动加入资产、显示耗时、失败重试及使用文本模型润色提示词后自动生图。
+ [新增] 图片聊天新增实景拼贴志与影像蒸馏志 Skill，支持只上传一张图片自动生成竖版编辑画册，并保存参考图供刷新恢复与失败重试。
+ [新增] 新增电商生图页面，支持商品图分镜规划、批量竖图生成、失败重试及详情页长图合成。
+ [优化] 电商生图遇到图片或分镜服务临时不可用时自动退避重试，并支持随时停止等待。
+ [修复] 电商生图为 WebP 商品图生成 PNG 规划副本，并在规划服务持续不可用时使用本地分镜继续生成。
+ [调整] 电商分图生成后不再等待下载到本地，生成状态立即结束，长图改为直接读取受限同源代理地址异步合成。
+ [新增] 电商分图支持以当前结果为参考单独修改提示词并重新生成，成功后自动使用最新分图重新合成长图。
+ [新增] 电商生图支持项目自动恢复、分镜排序删除、图片版本回退、独立文案层、平台宽度及长图分段打包导出。
+ [新增] 电商生图整合 25 个可智能匹配或手动选择的场景模板及真实风格变体，并支持反 AI 质感、图包目标、购买动机、品牌风格锁与可信证据。
+ [新增] 电商生图新增 Amazon、Shopify、TikTok Shop 等跨境渠道与目标市场、本地化文案、买家场景、Offer/CTA 和平台合规策略。
+ [修复] 修复跨境电商新增字段缺失时恢复本地项目导致页面白屏的问题。
+ [修复] 电商生图区分国内与跨境市场模式，国内模式不再发送默认 Amazon、海外市场或外语文案约束。
+ [优化] 电商分图改为生成完成后立即展示并通过单队列后台保存到本地，长图重合成复用同一素材且停止任务后不再接收迟到结果。
+ [调整] 应用品牌更名为未来画布，未来中转站与 QQ 群信息从导航栏移至社区公告弹窗和全局底栏。
+ [优化] 图片聊天、电商生图和生图工作台统一使用带扫光过渡的动态生成占位。
+ [修复] 修复远程图片已生成但受跨域限制无法保存到本地时被误判为生成失败的问题。
+ [新增] 适配 Adobe Firefly 图片与视频模型的 Chat Completions 调用格式。
+ [修复] 修复远程结果图或画布生图未可靠写入生成记录的问题。
+ [修复] 修复视频创作台生成方式切换导致前端类型检查和构建失败的问题。
+ [优化] 图片生成适配 OpenAI 兼容尺寸映射和宽高比参数。
+ [新增] 提示词来源增加自定义脚本抓取功能、支持新增来源。
+ [新增] 适配Grok图片生成和编辑。
+ [新增] 适配Grok视频生成、剪辑和扩展。
+ [新增] 未来中转站图片生成与编辑支持异步任务，未启用时自动回退同步接口。
+ [新增] Vite 开发与 Docker 部署为默认未来中转站提供同源 API 代理。
+ [修复] 修复 HTTPS 部署下未来中转站返回 HTTP 生成图片时被浏览器拦截的问题。
+ [调整] 右上角项目入口改为未来中转站，并新增QQ交流群公告。
+ [调整] 默认模型渠道及原有未修改的默认渠道改为未来中转站，API Key 由用户自行填写。
+ [调整] 默认 Docker Compose 改为基于当前源码本地构建镜像。
+ [修复] 优化Grok接口错误提示和视频结果CORS处理。
+ [修复] 修复首页和提示词库空封面触发的图片地址警告。
